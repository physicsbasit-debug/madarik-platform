...
class CloudSourceRepository:
    ...
    def save(self, source: CloudSource) -> CloudSource:
        ...
        with self._connect() as connection:
            ...
            if existing is not None and existing["id"] != source.id:
                previous = CloudSource.model_validate_json(existing["payload"])
                source.id = previous.id
                source.created_at = previous.created_at
                if source.owner_account_id is None:
                    source.owner_account_id = previous.owner_account_id
    ...
    def list(
        ...
        *,
        ...
        items = [
            CloudSource.model_validate_json(row["payload"])
            for row in rows
        ]
        ...
        if source_project_id:
            items = [
                item
                for item in items
                if item.source_project_id == source_project_id
            ]
        return items
    ...
    def get(self, source_id: str) -> CloudSource | None:
        with self._connect() as connection:
            row = connection.execute(
                "SELECT payload FROM cloud_sources WHERE id = ?",
                (source_id,),
            ).fetchone()
        if row is None:
            return None
        return CloudSource.model_validate_json(row["payload"])
